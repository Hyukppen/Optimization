...
# figure = plt.figure(figsize=[5.5, 9.5])
...
#     plt.axis([min(x1), max(x1), min(x2), max(x2)])
#     ax.set_zlim(0, 1)
#     figure.canvas.draw(); figure.canvas.flush_events(); plt.cla(); #time.sleep(0.01)
...
for l in range(50):
    ...
    figure.canvas.draw(); figure.canvas.flush_events(); time.sleep(.3)
